...
{
    /// <summary>
    /// Implements <see cref="IEnumerator{T}"/> for <see cref="StoreEntry{TKey, TValue}"/> items in a segment.
    /// </summary>
    private sealed class EntryEnumerator : IEnumerator<StoreEntry<TKey, TValue>>
    {
        ...
        /// </summary>
        /// <param name="driver">
        /// The <see cref="Driver{TKey, TValue}"/> on the segment of which to enumerate the entries.
        /// </param>
        /// <param name="cancellationToken">
        /// A <see cref="CancellationToken"/> value that can be used to cancel the operation.
        /// </param>
        public EntryEnumerator(Driver<TKey, TValue> driver, CancellationToken cancellationToken)
        {
            Debug.Assert(driver != null, "The driver must not be null.");
            Debug.Assert(driver.reader != null, "The driver's reader must not be null.");

            this.driver = driver;
            this.context = new ReadContext(
                driver.reader.OpenDataForReadAsync(0, null, cancellationToken).GetValueTaskResult());
        ...
        public bool MoveNext()
        {
            ...
            TKey key = driver.formatter.ReadKeyAsync(context.Stream, cancellationToken).GetValueTaskResult();
            if (!flags.HasFlag(EntryFlags.Deleted))
            {
                TValue value = driver.formatter.ReadValueAsync(context.Stream, cancellationToken).GetValueTaskResult();

                current = new StoreEntry<TKey, TValue>(key, value);
            }
            else
            {
                // The flags indicate that the entry represents a deleted key, so there wouldn't be a value to read.
