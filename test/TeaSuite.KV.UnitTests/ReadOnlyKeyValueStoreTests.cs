...
public sealed class ReadOnlyKeyValueStoreTests
{
    ...
    public void TryGetFindsNothingInZeroSegments(int key)
    {
        ...
        ReadOnlyKeyValueStore<int, int> store = new(
            NullLogger<ReadOnlyKeyValueStore<int, int>>.Instance,
    ...
    {
        ReaderContext reader1 = CreateSegmentReader(0, 49,
            EntryFlags.None,
            expectedValue.HasValue ? EntryFlags.None : EntryFlags.Deleted,
            EntryFlags.None);
        ReaderContext reader2 = CreateSegmentReader(50, 99,
            EntryFlags.None,
            expectedValue.HasValue ? EntryFlags.None : EntryFlags.Deleted,
            EntryFlags.None);
        Mock<IEntryFormatter<int, int>> mockEntryFormatter = new(MockBehavior.Strict);

        mockEntryFormatter
            .SetupSequence(f => f.ReadKeyAsync(reader1.IndexStream, default))
            .ReturnsAsync(0)
            .ReturnsAsync(49)
            .ThrowsAsync(new EndOfStreamException());
        mockEntryFormatter
            .SetupSequence(f => f.ReadKeyAsync(reader2.IndexStream, default))
            .ReturnsAsync(50)
            .ReturnsAsync(99)
            .ThrowsAsync(new EndOfStreamException());

        mockEntryFormatter
            .SetupSequence(f => f.ReadKeyAsync(reader1.DataStream, default))
            .ReturnsAsync(0)
            .ReturnsAsync(25)
            .ReturnsAsync(49)
            .ThrowsAsync(new EndOfStreamException());
        ...
        mockEntryFormatter
            .SetupSequence(f => f.SkipReadValueAsync(reader1.DataStream, default))
            .Returns(new ValueTask())
            .ThrowsAsync(new EndOfStreamException());
        if (expectedValue.HasValue && key < 50)
        {
            mockEntryFormatter
                .SetupSequence(f => f.ReadValueAsync(reader1.DataStream, default))
                .ReturnsAsync(expectedValue.Value)
                .ThrowsAsync(new EndOfStreamException());
        }

        mockEntryFormatter
            .SetupSequence(f => f.ReadKeyAsync(reader2.DataStream, default))
            .ReturnsAsync(50)
            .ReturnsAsync(75)
            .ReturnsAsync(99)
            .ThrowsAsync(new EndOfStreamException());
        // The first entry is skipped.
        mockEntryFormatter
            .SetupSequence(f => f.SkipReadValueAsync(reader2.DataStream, default))
            .Returns(new ValueTask())
            .ThrowsAsync(new EndOfStreamException());
        if (expectedValue.HasValue && key >= 50)
        {
            mockEntryFormatter
                .SetupSequence(f => f.ReadValueAsync(reader2.DataStream, default))
                .ReturnsAsync(expectedValue.Value)
                .ThrowsAsync(new EndOfStreamException());
        }

        Segment<int, int> seg1 = CreateSegment(
            1, reader1.Reader.Object, mockEntryFormatter.Object);
        Segment<int, int> seg2 = CreateSegment(
            2, reader2.Reader.Object, mockEntryFormatter.Object);
        Mock<ISegmentManager<int, int>> mockSegmentManager = CreateSegmentManager(
            seg1, seg2);
        ReadOnlyKeyValueStore<int, int> store = new(
            NullLogger<ReadOnlyKeyValueStore<int, int>>.Instance,
    ...
    public void SetThrowsNotSupportedException()
    {
        ...
        NotSupportedException ex = Assert.Throws<NotSupportedException>(
            () => store.Set(1, 2));
        Assert.Equal("Setting values is not supported in a ReadOnlyKeyValueStore.",
            ex.Message);
    }
    ...
    public void DeleteThrowsNotSupportedException()
    {
        ...
        NotSupportedException ex = Assert.Throws<NotSupportedException>(
            () => store.Delete(1));
        Assert.Equal("Deleting values is not supported in a ReadOnlyKeyValueStore.",
            ex.Message);
    }
    ...
    {
        ReaderContext reader1 = CreateSegmentReader(
            0, 49, EntryFlags.None, EntryFlags.None);
        ReaderContext reader2 = CreateSegmentReader(
            50, 99, EntryFlags.None, EntryFlags.None);
        Mock<IEntryFormatter<int, int>> mockEntryFormatter = new(MockBehavior.Strict);
    ...
    {
        ReaderContext reader1 = CreateSegmentReader(
            0, 49, EntryFlags.None, EntryFlags.Deleted);
        ReaderContext reader2 = CreateSegmentReader(
            50, 99, EntryFlags.None, EntryFlags.None);
        Mock<IEntryFormatter<int, int>> mockEntryFormatter = new(MockBehavior.Strict);
    ...
    public void GetEnumeratorWithRangeWorks()
    {
        ...
        mockEntryFormatter
            .SetupSequence(f => f.ReadValueAsync(reader2.DataStream, default))
            .ReturnsAsync(2)
            .ReturnsAsync(3).ThrowsAsync(new EndOfStreamException());
    ...
    {
        ReaderContext reader1 = CreateSegmentReader(
            0, 49, 3, EntryFlags.None, EntryFlags.None);
        ReaderContext reader2 = CreateSegmentReader(
            50, 99, 3, EntryFlags.None, EntryFlags.None);
        Mock<IEntryFormatter<int, int>> mockEntryFormatter = new(MockBehavior.Strict);
